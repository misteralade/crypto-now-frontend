...
import type { KycSessionResponse, KycStatusResponse } from "../../types/kyc.types.ts";
...
const CHECKLIST = [
  { key: "id_selected", label: "ID Type Selected" },
  { key: "front", label: "Front Document Uploaded" },
  { key: "selfie", label: "Selfie Captured" },
  { key: "submitted", label: "Submitted for Verification" },
  { key: "verifying", label: "Verifying with Didit…" },
];

export default function KycProcessingStep({ session, onComplete }: KycProcessingStepProps) {
  const [fallbackPolling, setFallbackPolling] = useState(false);

  // Long-poll until status changes from 'processing'
  const { isPolling } = useKycLongPoll({
    enabled: true,
    onStatusChange: (status) => {
      if (status.currentStep !== "processing") {
        onComplete(status);
      }
    },
    onError: () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (success && data && data.currentStep !== "processing") {
  ...
  const completedKeys = new Set<string>([
    "id_selected",
    "front",
    session.selectedIdType !== "passport" ? "back" : null,
    "selfie",
    "submitted",
    ...
    <div className="space-y-8 py-4">
      ...
      <div className="space-y-3">
        {CHECKLIST.map((item) => {
          ...
          const isActive = item.key === "verifying" && (isPolling || fallbackPolling);
      ...
      <p className="text-center text-xs text-gray-400">
        Typically completes within 30 seconds. If it takes longer, we'll notify you by email.
      </p>
    </div>
  );
}
