...
export function TransactionFilters({ onClose, filters, onFiltersChange, supportedCrypto }: TransactionFiltersProps) {
  ...
  const handleReset = () => {
    const resetFilters = { fromDate: undefined, toDate: undefined, cryptocurrency: undefined, status: undefined }
    setLocalFilters(resetFilters)
    onFiltersChange(resetFilters)
    onClose()
  }

  const statusOptions = ['AWAITING_PAYMENT', 'IN_REVIEW', 'AWAITING_CRYPTO', 'DEPOSIT_DETECTED', 'PENDING_CONFIRMATION', 'DEPOSIT_PENDING_MINIMUM', 'DEPOSIT_CONFIRMED', 'PAYOUT_INITIATED', 'PENDING_PAYOUT', 'PAYOUT_FAILED', 'COMPLETED', 'FAILED', 'CANCELLED', 'EXPIRED', 'DISPUTED', 'REFUNDED'];
          ...
          <div className="relative">
            ...
            >
              ...
              {statusOptions.map((status: string) => (
                <option key={status} value={status.toUpperCase()}>{status.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, c => c.toUpperCase())}</option>
              ))}
            </select>
            <ChevronDown className="absolute right-3 top-1/2 transform -translate-y-1/2 text-desc w-4 h-4 pointer-events-none" />
          </div>
