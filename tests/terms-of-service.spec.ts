...
test.describe('Terms of Service Page', () => {
  ...
  test('should display terms of service page', async ({ page }) => {
    try {
      ...
      await page.waitForURL(/\/terms-of-service/, { timeout: 10000 });
      await page.waitForLoadState('load');
    } catch {
      await page.waitForLoadState('load');
      // Verify we're on terms-of-service page
      const currentUrl = page.url();
      if (!currentUrl.includes('/terms-of-service')) {
        await page.goto('/terms-of-service', { waitUntil: 'domcontentloaded', timeout: 10000 });
        await page.waitForURL(/\/terms-of-service/, { timeout: 10000 });
      }
    }
    ...
    const heading = page.locator('h1, h2').filter({ hasText: /terms|service|agreement/i });
